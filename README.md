...
pnpm-монорепа: Vite/React клиент (Farcaster Mini App) + Hono API + Postgres + Redis + Go reconciler.
...
- S3-совместимое хранилище хранит PNG-карточки, которые рендерит API при шеринге
...
- `S3_ENDPOINT`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_BUCKET` — S3-совместимое хранилище для PNG-картинок шеринга, virtual-hosted style. Регион хардкодится `"auto"` (S3-совместимые провайдеры его игнорят), публичный URL собирается сам как `https://${S3_BUCKET}.${endpoint-host}/${key}`. Без этих переменных эндпоинт `/api/share/create` отвечает 503.
