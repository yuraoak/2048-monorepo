...
async function migrate() {
  // Schema is rebuilt from scratch every run — service is wiped & redeployed
  ...
  await sql`DROP TABLE IF EXISTS undo_credits CASCADE`;
  await sql`DROP TABLE IF EXISTS pack_intents CASCADE`;
  await sql`DROP TABLE IF EXISTS reconciler_cursor CASCADE`;
  await sql`DROP TABLE IF EXISTS shares CASCADE`;
  ...
  `;
  await sql`CREATE INDEX scores_score_idx ON scores (score DESC)`;
  ...
  `;
  await sql`CREATE INDEX pack_intents_fid_idx ON pack_intents (fid)`;

  // Single-row table tracking the last Base block fully scanned by the
  // reconciler worker. A primary key check pins it to id=1.
  await sql`
    CREATE TABLE reconciler_cursor (
      id INTEGER PRIMARY KEY CHECK (id = 1),
      last_block BIGINT NOT NULL DEFAULT 0,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `;
  await sql`INSERT INTO reconciler_cursor (id, last_block) VALUES (1, 0)`;
  ...
  `;
  await sql`CREATE INDEX shares_fid_idx ON shares (fid, created_at DESC)`;
